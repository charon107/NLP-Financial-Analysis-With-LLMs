...
URL = 'https://www.moneycontrol.com/india/stockpricequote/online-services/one97communicationspaytm/OC03'
resp = requests.get(generate_new_url(URL), headers=hdr)
soup = BeautifulSoup(resp.text, 'html.parser')
script_tag = soup.find('script', string=re.compile('var tophold_jsn'))

if script_tag:
    # 微调正则表达式，确保正确提取JSON字符串
    # 这里假设JSON字符串以单引号或双引号包裹，并确保匹配到分号之前（;），以避免捕获不完整的JSON
    match = re.search(r'var tophold_jsn = (\'{.*?}\')|(\"{.*?}\");', script_tag.string, re.DOTALL)

    if match:
        # 提取匹配的JSON字符串，并去掉两端的引号
...
# 打开一个CSV文件准备写入
with open(r'D:\Program Files (x86)\Python\PythonProject\FYP\Financial Report\Signle_Companies_ShareHoder_Sheet.csv', 'w', newline='', encoding='utf-8') as csvfile:
