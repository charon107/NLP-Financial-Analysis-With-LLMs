...
def construct_cash_flow_sheet_url(base_url):
    # 分割基础URL以获取公司名称和代码
    ...
    # 构建balance sheet的URL
    cash_flow_sheet_url = f"https://www.moneycontrol.com/financials/{company_name}/cash-flowVI/{code}#{code}"
    return cash_flow_sheet_url


# 修改脚本来处理每个URL

def write_csv_for_year(year, data, csv_header):
    csv_file_path = f'D:\Program Files (x86)\Python\PythonProject\FYP\Presentation\Financial_Parameters_Colloction\Cash&Flow_Sheet\Company_Cash&Flow_Sheet_{year}.csv'
    # 检查文件是否存在，以决定是否写入表头
...
def process_urls(urls):
    for url in urls:
        try:
          ...
          resp = requests.get(construct_cash_flow_sheet_url(url), headers=hdr)
