...
def construct_profit_loss_sheet_url(base_url):
    # 分割基础URL以获取公司名称和代码
    ...
    # 构建balance sheet的URL
    profit_loss_sheet_url = f"https://www.moneycontrol.com/financials/{company_name}/profit-lossVI/{code}#{code}"
    return profit_loss_sheet_url


# 修改脚本来处理每个URL

def write_csv_for_year(year, data, csv_header):
    csv_file_path = f'D:\Program Files (x86)\Python\PythonProject\FYP\Presentation\Financial_Parameters_Colloction\Profit&Loss_Sheet\Company_profit_loss_Sheet_{year}.csv'
    # 检查文件是否存在，以决定是否写入表头
...
def process_urls(urls):
    for url in urls:
        try:
          ...
          resp = requests.get(construct_profit_loss_sheet_url(url), headers=hdr)
