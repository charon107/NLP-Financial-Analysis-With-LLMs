...
def construct_balance_sheet_url(base_url):
    ...
    # 构建balance sheet的URL
    balance_sheet_url = f"https://www.moneycontrol.com/financials/{company_name}/balance-sheetVI/{code}#{code}"
...
def write_csv_for_year(year, data, csv_header):
    csv_file_path = f'D:\Program Files (x86)\Python\PythonProject\FYP\Financial Parameters\Balance_Sheet\Company_Balance_Sheet_{year}.csv'
    # 检查文件是否存在，以决定是否写入表头
