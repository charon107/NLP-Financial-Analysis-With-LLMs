...
URL = 'https://www.moneycontrol.com/india/stockpricequote/miningminerals/nirajispatindustries/NII03'
...
#
#
# # Function to write a CSV file for a given year
# def write_csv_for_year(year_index, year):
#     csv_file_path = f'D:\Program Files (x86)\Python\PythonProject\FYP\Financial Report\Company_Balance_Sheet_{year}.csv'
#     with open(csv_file_path, 'w', newline='', encoding='utf-8') as file:
#         writer = csv.writer(file)
#         writer.writerow(csv_header)  # Write the headers
#
#         # Write the data for the Symbol and Company Name
#         symbol = table_data[0][1]
#         company_name = table_data[1][1]
#         data_row = [symbol, company_name]
#
#         # Append the data for the current year
#         for row in table_data[4:]:
#             data_row.append(row[year_index + 1] if len(row) > year_index + 1 else '')
#
#         # Write the single data row to the CSV
...
#     csv_file_path = write_csv_for_year(i, year)
#     csv_file_paths.append(csv_file_path)
#
# # The paths to the created CSV files
# csv_file_paths
# print(table_data)
# column_names = ['Symbol', 'Company Name'] + [item[0] for item in table_data if item[0] and not item[0].startswith('Balance Sheet')]
# # 添加年份作为列名
# years = table_data[2][1:6]
# column_names += years
#
# # 初始化数据行
# rows = []
# for _ in range(len(years)):
#     rows.append([table_data[0][1], table_data[1][1]] + [''] * (len(column_names) - 2))
#
# # 填充数据行
# for data in table_data:
#     if data[0] and not data[0].startswith('Balance Sheet'):
#         for i, value in enumerate(data[1:6]):
#             column_index = column_names.index(data[0])
#             rows[i][column_index] = value
#
# # 写入CSV文件
# csv_file_path = r'D:\Program Files (x86)\Python\PythonProject\FYP\Financial Report\financial_data.csv'
